...
bool isModifier(const KeyEvent& e)
{
    return (e.Key==KeyCodes::KEY_LeftAlt) || (e.Key==KeyCodes::KEY_LeftMeta) || (e.Key==KeyCodes::KEY_LeftShift) ||
            (e.Key==KeyCodes::KEY_LeftControl) || (e.Key==KeyCodes::KEY_RightAlt) || (e.Key==KeyCodes::KEY_RightMeta) ||
            (e.Key==KeyCodes::KEY_RightShift) || (e.Key==KeyCodes::KEY_RightControl);
}

CGEventFlags modifierFlag(KeyEvent e)
{
    switch (e.Key)
    {
    case KEY_LeftAlt:
...
static std::vector<CGEventFlags> k_flagStack;
static CGEventSourceRef k_lastEventSourceRef;

void releaseEvent(CGEventSourceRef source, CGKeyCode key, bool pressed, std::vector<CGEventFlags> flags)
{    CGEventRef event = CGEventCreateKeyboardEvent (source, key, pressed);
    for(auto flag : flags)
    {
        CGEventSetFlags(event, flag | CGEventGetFlags(event));
    }
...
void SendInput(const KeyEvent &e)
{
    if(e.Pressed)
    {
        if(isModifier(e))
        {
            k_flagStack.push_back(modifierFlag(e));
        }
        else
        {
            k_lastEventSourceRef = CGEventSourceCreate(kCGEventSourceStateCombinedSessionState);
            releaseEvent(k_lastEventSourceRef, ConvertToNative(e.Key), e.Pressed, k_flagStack);
        }
    }
    else
    {
        if( isModifier(e) )
        {
            CFRelease(k_lastEventSourceRef);
            k_lastEventSourceRef = nullptr;
            k_flagStack.clear();
        }
        else
        {
